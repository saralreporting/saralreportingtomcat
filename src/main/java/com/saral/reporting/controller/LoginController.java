...
@Transactional
@Controller
@SessionAttributes({ "sign_no", "user_id", "user_name", "hm", "department_level_name", "department_id", "saralUserServiceList",
	"designation_id", "designation_name","location_Id","deptidwithName","filterString","filterbserviceId","filterbdeptId","filterbdisttId" })
...
//	private static final Logger LOGGER = LoggerFactory.getLogger(LoginController.class);
	...
	public String homePage(HttpServletRequest request,ModelMap model,Pageable pageable) {
		...
		List<DashBoardReportData> listReportData = dashBoardReportDataService.findDistinctTop5ByDepartmentIdOrderByCountDesc(department_id);
		...
		*/
		...
		List<SpeServiceCoverageDetails> servicesForSaralUser = speServiceCoverageDetailsService.findBySpdiDesigOfcrUserid(user_id);
		...
		return "welcome";}
			 ...
			 * 
			 * 
			 * List<String> bserviceId = new ArrayList<String>();
				List<String> bdisttId = new ArrayList<String>();
				List<String> bdeptId = new ArrayList<String>();
			 * 
			 * 
			 * 
			 * 
			 * for(ReportDomainMaster rp : reportDomainMaster){
				...
				if(rpdmnId==1L){
					bdeptId.add(rp.getDomainValueId().toString());
				}else if(rpdmnId==2L){
					bdisttId.add(rp.getDomainValueId().toString());
				}else if(rpdmnId==3L){
			...
			/*String listString="";
			if(bdeptId.size()>0){
				listString = String.join(", ", bdeptId);
			}
			
			String listString2 = "";
			if(bdisttId.size()>0){
				listString2 = String.join(", ", bdisttId);
			}
			
			String listString3 = "";
			if(bserviceId.size()>0){
				 listString3 = String.join(", ", bserviceId);
			}
			
			String completeFilter = "";
			if(listString!=""){
				listString = "department_id IN (" + listString + ")";
				completeFilter = "Where " + listString;
			}
		
			
			 * code for distric filters	
			if((listString2!="") &&(completeFilter!="")){
				listString2 = " AND district_id IN (" + listString2 + ")";
				completeFilter = completeFilter + listString2;
			}else if((listString2!="") &&(completeFilter=="")){
				listString2 = "district_id IN (" + listString2 + ")";
				completeFilter = "Where " + listString2;
			}
			
			if((listString3!="") &&(completeFilter!="")){
				listString3 = " AND base_service_id IN (" + listString3 + ")";
				completeFilter = completeFilter + listString3;
			}else if((listString3!="") &&(completeFilter=="")){
				listString3 = "base_service_id IN (" + listString3 + ")";
				completeFilter = "Where " + listString3;
			}
			
			System.out.println("My Final Filter is " + completeFilter);
			//Code for filter String ends here
			model.put("filterString", completeFilter);*/
