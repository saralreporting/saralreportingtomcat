...
public interface TaskMappingMasterRepository extends JpaRepository<TaskMappingMaster, Long> {
	...
	@Query(value="SELECT   count(*) from saral.taskmappingmaster where left(cast(service_id as varchar) ,LENgth(cast(service_id as varchar)) - 4) =?1 and version_no =?2", nativeQuery =true)
